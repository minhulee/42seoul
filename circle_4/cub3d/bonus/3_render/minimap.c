...
/*   Updated: 2024/09/12 10:02:51 by minhulee         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void	make_line(t_cub3d *info, int y, int x)
{
	...
	color = (1 << 24) | (051 << 16) | (051 << 8) | 051;
	if (x % MINIMAP_L == 0)
		((int *)info->screen.addr)[y * WINDOW_W + x] = color;
	if (y % MINIMAP_L == 0)
		((int *)info->screen.addr)[y * WINDOW_W + x] = color;
	if (x == MINIMAP_W - 1)
		((int *)info->screen.addr)[y * WINDOW_W + x] = color;
