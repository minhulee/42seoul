...
/*   Updated: 2024/09/12 12:33:31 by minhulee         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d_bonus.h"

// ㅈㅣㄲㅏㄱ으로 방사되는 레이 한개로 감지
...
void	tex_ver_line(t_cub3d *info, t_var *vars, int start, int x)
{
	...
	vars->tex_pos = ((start - WINDOW_H / 2 + vars->line_h / 2) - info->camera.tov) * vars->step;
	i = 0;
	while (i < WINDOW_H)
	{
		if (i < start)
...
void	select_wall(t_tile **map, t_player *player, t_var *vars, t_ray *ray)
{
	if (ray->is_door)
		vars->tex_num = DR;
	...
	t_player *player, t_camera *camera)
{
	...
	while (++x < WINDOW_W)
	{
		...
		select_wall(info->map_data.map, player,&vars, ray);
