...
/*   Updated: 2024/09/20 19:33:24 by minhulee         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void	convert_to_w_f_c(t_cub3d *info, t_map_data *map_data,
	char *line, int fd)
{
	while (1)
	{
		...
		else if (!map_data->walls[DR] && !ft_strncmp(line, "DR ", 3))
			load_xpm(info, &(map_data->walls[DR]), remove_space(line + 2), 64);
...
void	parsing(t_cub3d *info, char *file)
{
	...
	convert_to_w_f_c(info, &info->map_data, NULL, quick_open_file(file, 0));
