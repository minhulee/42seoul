...
/*   Updated: 2024/09/20 16:32:52 by minhulee         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	is_out_of_map(t_player *player, t_map_data *map_data)
{
	...
	if ((player->map_y < 0 || player->map_y >= map_data->height)
		|| player->map_x < 0 || player->map_x >= map_data->width)
		return (TRUE);
	dst = map_data->map[player->map_y][player->map_x];
	if (dst == WALL || dst == EMPTY)
		return (TRUE);
	return (FALSE);
}
...
void	execute_dda(t_ray *ray, t_player *player, t_map_data *map_data)
{
	...
	while (!ray->is_hit)
	{
		...
		if (is_out_of_map(player, map_data))
			ray->is_hit = TRUE;
	}
}
